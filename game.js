...
const SPIN = new function () {
    ...
    //canvas, context, width and height of the playing field
    SPIN.start = (W, H) => {
        cnv = $('cnv');
...
window.addEventListener('load', function () {
    ...
    for (let j = 0; j < 3; j++) {
        for (let i = 0; i < 10; i++) {
            enemies.push(SPIN.create_node(30 + (20 + 40) * i, 20 + (20 + 40) * j, 40, 40, '#EB6D06', enemy_ai));
        }
    }
    ...
    //Node size and position, x-y position, w-h size
    SPIN.create_node(640 / 2 - 25, 480 - 50 - 30, 50, 50, '#00C618', (node) => {
        if (SPIN.key('KeyA'))
            node.x -= 1;
        if (SPIN.key('KeyD'))
            node.x += 1;
